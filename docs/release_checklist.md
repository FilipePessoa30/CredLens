...
1. Do NOT change any tracked source file after this point until step 5 - every evidence snapshot below is stamped with a source-snapshot fingerprint (`credlens.release.source_snapshot`), and a later edit makes ALL of them stale at once.
...
3. Run `credlens release licenses` and `credlens release sbom` (each now WRITES its own `reports/release/{license_inventory,sbom.cyclonedx}.json`, not just prints), then `credlens release manifest --visual-qa-status <status> --docker-status <status> --ci-status <status>` with the REAL, just-observed status of this environment - never a guessed or aspirational value.
4. Run `credlens release checksums` **LAST**, only after step 3's three files have their final bytes on disk - it (re)writes `reports/release/SHA256SUMS` from the current content of the canonical release-asset set (`credlens.release.checksums.CANONICAL_RELEASE_ASSETS`: `sbom.cyclonedx.json`, `release_manifest.json`, `license_inventory.json`) - these are exactly the files attached to the GitHub Release. Never hand-edit this file or run a one-off `sha256sum` command outside this tool again - that is exactly how the `v1.0.0rc2` Pre-Release ended up with a stale committed `SHA256SUMS` in the first place (Fase 12; see `credlens.release.checksums`'s module docstring for the full incident history).
5. Read `reports/release/release_manifest.json`'s `readiness_decision` and `release_blockers` - if `release_blockers` is non-empty, the decision MUST be `release_candidate_not_ready`, regardless of how much other work is complete.
6. Record the decision, the blocking-gate results, and every known limitation in the final phase report - never omit a failing gate to make the release look more complete than it is.
