...
This document describes the **target** architecture for the full CredLens project. It states plainly, section by section, what exists today versus what is planned. Nothing described as "planned" below is implemented yet unless stated otherwise - as of Phase 5, `Transform` and `Warehouse` below have moved from planned to implemented (see `docs/warehouse_architecture.md`); as of Phase 6, the KPI/vintage/roll-rate/scenario-comparison parts of `Analytics` gained a reproducible, versioned analysis layer on top (see `docs/analysis_architecture.md`); as of Phase 7, the demo-app half of `Presentation` is implemented as a Streamlit Decision Intelligence Dashboard (see `dashboard/README.md`); as of Phase 8, an interpretable behavioral risk model exists (`credlens.modeling`, see `reports/modeling/technical_report.md`) but trained on the real public UCI benchmark, deliberately not on the synthetic warehouse; as of Phase 9, that model has an independent validation layer (`credlens.model_validation`) and a clearly labeled monitoring-simulation layer (`credlens.monitoring`, never a real production monitoring system) - see `reports/model_validation/README.md`/`reports/monitoring/README.md` - a policy/cutoff simulator and the Power-BI-specific half of `Presentation` remain planned.
