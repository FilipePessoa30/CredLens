[Read in English](README.md) · [Resumo de portfólio em 2 minutos](PORTFOLIO.pt-BR.md) · [Resumo de uma página para recrutadores](docs/recruiter_brief.pt-BR.md) · [Guia de perguntas de entrevista](docs/interview_guide.pt-BR.md)
...
├── docs/                         # Documentação de negócio, arquitetura, aquisição de dados, contratos, gerador, warehouse, análise, além de recruiter_brief/interview_guide/release_checklist (Fase 10)
...
- **Versionado**: todo o código-fonte, SQL, config, testes, docs, o seed do dbt ([`warehouse/seeds/dim_dpd_bucket.csv`](warehouse/seeds/dim_dpd_bucket.csv) — dado de referência pequeno e estático, não dado gerado/adquirido), os artefatos do modelo candidato oficial (`reports/modeling/models/*.joblib`) e as 8 capturas de tela do dashboard (`docs/assets/dashboard/`).
